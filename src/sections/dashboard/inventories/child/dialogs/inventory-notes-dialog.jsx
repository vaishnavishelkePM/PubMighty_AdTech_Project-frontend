...
export default function InventoryNotesDialog({ open, inventory, headers, onClose }) {
  ...
  const fetchNotes = async (inventoryId) => {
    if (!inventoryId) return;

    try {
      setNotesLoading(true);
      const url = `${CONFIG.apiUrl}/v1/admin/inventory/${inventoryId}/notes`;
      ...
      console.error('fetchInventoryNotes error:', err);
      setNotes([]);
    } finally {
  ...
  const handleSaveNote = async () => {
    ...
    try {
      ...
      if (editingNoteId) {
        url = `${CONFIG.apiUrl}/v1/admin/inventory/notes/${editingNoteId}`;
        method = 'put';
        payload = { note: trimmed, assignedTo };
      } else {
        url = `${CONFIG.apiUrl}/v1/admin/inventory/${inventory.id}/notes`;
        method = 'post';
        payload = { inventoryId: inventory.id, note: trimmed, assignedTo };
      }
      ...
      if (resp.data?.success) {
        // Re-fetch so we get writer + assignee objects populated
        await fetchNotes(inventory.id);

        resetNoteForm();
        setEditorOpen(false);
      }
    } catch (err) {
      console.error('handleSaveInventoryNote error:', err);
    } finally {
  ...
  const handleDeleteNote = async (note) => {
    ...
    try {
      const url = `${CONFIG.apiUrl}/v1/admin/inventory/notes/${note.id}`;
      ...
      console.error('handleDeleteInventoryNote error:', err);
    }
  };
  ...
  const renderNotesList = () => {
    ...
    if (!sortedNotes.length) {
      return (
        <Typography variant="body2" color="text.secondary">
          No notes for this inventory yet.
        </Typography>
      );
    }
  ...
  const inventoryTitle =
    inventory?.name || (inventory?.id ? `Inventory #${inventory.id}` : 'Inventory');
      ...
      >
        ...
        >
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
            ...
            <Typography variant="h6">Inventory Notes</Typography>
            {inventory && (
              <Typography variant="body2" color="text.secondary">
                {`— ${inventoryTitle}`}
              </Typography>
      ...
      >
        ...
        <DialogContent dividers sx={{ pt: 2 }}>
          ...
          <Editor
            ...
            fullItem
            placeholder={
              editingNoteId ? 'Update your note...' : 'Write an internal note for this inventory...'
            }
          />
