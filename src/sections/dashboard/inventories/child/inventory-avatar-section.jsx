  ...
  const handleCropSave = async () => {
    ...
    try {
      ...
      if (!inventoryId) {
        ...
        setIsUploading(false); // ✅ IMPORTANT: reset here before return
        return;
      }

      // 🚩 EDIT MODE: inventoryId exists → upload immediately to backend
      const formData = new FormData();
      formData.append('logo', croppedBlob, 'inventory-logo.jpg');
      ...
      const resp = await axios.post(urlApi, formData, {
        headers: {
          Authorization: `Bearer ${token}`,
          // no need to manually set Content-Type; axios handles FormData
        },
        validateStatus: () => true,
      });
      ...
      if (resp.data?.success) {
        const newLogo = resp.data?.data?.logo || resp.data?.data?.avatar;
        ...
        if (!newLogo) {
          ...
          onPreviewChange?.(newPreview);
          onAvatarFilenameChange?.(newLogo);
          onCroppedBlobChange?.(null); // in edit mode we don’t keep the blob
