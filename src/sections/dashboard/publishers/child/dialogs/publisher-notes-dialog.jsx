...
import AdminSelector from 'src/components/selectors/Admin1/admin-selector';

// publisher: { id, username, ... }

export default function PublisherNotesDialog({ open, publisher, headers, onClose }) {
  const [notes, setNotes] = useState([]);
  const [notesLoading, setNotesLoading] = useState(false);
  const [notesSaving, setNotesSaving] = useState(false);

  const [noteText, setNoteText] = useState('');
  const [noteAssignedTo, setNoteAssignedTo] = useState(''); // admin id as string
  ...
  // ---- Shared fetch function so we can call after save as well ----
  const fetchNotes = async (publisherId) => {
    if (!publisherId) return;

    try {
      setNotesLoading(true);
      const url = `${CONFIG.apiUrl}/v1/admin/publishers/${publisherId}/notes`;
      ...
      console.error('fetchPublisherNotes error:', err);
      setNotes([]);
    } finally {
  ...
  // Fetch notes when dialog opens / publisher changes
  useEffect(() => {
    if (!open || !publisher?.id) {
      setNotes([]);
      resetNoteForm();
      setEditorOpen(false);
      return;
  ...
  const handleSaveNote = async () => {
    ...
    try {
      ...
      if (editingNoteId) {
        // EDIT
        url = `${CONFIG.apiUrl}/v1/admin/publishers/notes/${editingNoteId}`;
        method = 'put';
        payload = {
          note: trimmed,
          assignedTo,
        ...
        // CREATE
        url = `${CONFIG.apiUrl}/v1/admin/publishers/${publisher.id}/notes`;
        method = 'post';
        payload = {
      ...
      if (resp.data?.success) {
        // 🔥 IMPORTANT: re-fetch notes so we get writer + assignee objects
        await fetchNotes(publisher.id);

        resetNoteForm();
        setEditorOpen(false);
      }
    } catch (err) {
      console.error('handleSaveNote (publisher) error:', err);
    } finally {
  ...
  const handleDeleteNote = async (note) => {
    ...
    try {
      const url = `${CONFIG.apiUrl}/v1/admin/publishers/notes/${note.id}`;
      ...
      if (resp.data?.success) {
        // You can filter locally OR re-fetch; filter is fine here
      ...
      console.error('handleDeleteNote (publisher) error:', err);
    }
  };
  ...
  const renderNotesList = () => {
    ...
    if (!sortedNotes.length) {
      return (
        <Typography variant="body2" color="text.secondary">
          No notes for this publisher yet.
        </Typography>
      );
    }
  ...
  return (
    <>
      ...
      <Dialog
        open={open}
        onClose={undefined} // prevent backdrop close
        fullWidth
        maxWidth="md"
      ...
      >
        ...
        >
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
            ...
            <Typography variant="h6">Publisher Notes</Typography>
            {publisher && (
              <Typography variant="body2" color="text.secondary">
                {publisher.username ? `— ${publisher.username}` : `— Publisher #${publisher.id}`}
              </Typography>
            )}
          </Box>
      ...
      >
        ...
        <DialogContent dividers sx={{ pt: 2 }}>
          ...
          <Editor
            ...
            fullItem
            placeholder={
              editingNoteId ? 'Update your note...' : 'Write an internal note for this publisher...'
            }
          />
